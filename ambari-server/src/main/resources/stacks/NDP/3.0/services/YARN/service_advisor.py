...
class MAPREDUCE2Recommender(YARNRecommender):
  ...
  def recommendMapReduce2ConfigurationsFromHDP22(self, configurations, clusterData, services, hosts):
    ...
    putMapredProperty('yarn.app.mapreduce.am.command-opts', "-Xmx" + str(int(0.8 * int(configurations["mapred-site"]["properties"]["yarn.app.mapreduce.am.resource.mb"]))) + "m" + " -Dndp.version=${ndp.version}")
    servicesList = [service["StackServices"]["service_name"] for service in services["services"]]
