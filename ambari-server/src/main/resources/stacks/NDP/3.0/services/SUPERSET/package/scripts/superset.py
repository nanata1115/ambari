...
class Superset(Script):
    ...
    def configure(self, env, upgrade_type=None):
        ...
        File(os.path.join(params.superset_config_dir, "superset_config.py"),
             owner=params.superset_user,
             group=params.user_group,
             mode=0755,
             content=Template("superset_config.py.j2")
             )
        ...
        Execute(format(
            "source {params.superset_config_dir}/superset-env.sh ; {params.superset_bin_dir}/superset fab "
            "create-admin  --username '{params.superset_admin_user}' --password '{params.superset_admin_password!p}' "
            "--firstname '{params.superset_admin_firstname}' --lastname '{params.superset_admin_lastname}' --email '{"
            "params.superset_admin_email}'"),
                user=params.superset_user)
        ...
        if len(params.druid_coordinator_hosts) > 0:
            Execute(format(
                "source {params.superset_config_dir}/superset-env.sh ; {params.superset_bin_dir}/superset "
                "configure_druid_cluster --name druid-ambari --coordinator-host {params.druid_coordinator_host} "
                "--coordinator-port {params.druid_coordinator_port} --broker-host {params.druid_router_host} "
                "--broker-port {params.druid_router_port} --coordinator-endpoint druid/coordinator/v1 "
                "--broker-endpoint druid/v2"),
                    user=params.superset_user)

            generate_logfeeder_input_config('superset',
                                            Template("input.config-superset.json.j2", extra_imports=[default]))
