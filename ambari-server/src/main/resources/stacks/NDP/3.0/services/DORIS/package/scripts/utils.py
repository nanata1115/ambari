...
def get_master_host(host):
    ...
    leader_ip = commands.getoutput(
        "mysql -h {0} -P {1} -u {2} -p{3} -e \"SHOW FRONTENDS;\" | awk '$9==\"true\" {{print $2}}'".format(host,
                                                                                                           port,
                                                                                                           user,
                                                                                                           password))
    Logger.info("get master with password  +{leader_ip}")
    if leader_ip.find("ERROR") >= 0:
        return commands.getoutput(
            "mysql -h {0} -P {1} -u {2} -e \"SHOW FRONTENDS;\" | awk '$9==\"true\" {{print $2}}'".format(host, port,
                                                                                                         user))
        Logger.info("get master with out password  +{leader_ip}")
    else:
        return leader_ip
