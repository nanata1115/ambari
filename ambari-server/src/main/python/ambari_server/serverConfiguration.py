...
class ServerConfigDefaultsLinux(ServerConfigDefaults):
  def __init__(self):
    ...
    self.NR_ADJUST_OWNERSHIP_LIST = [
      ...
      (AmbariPath.get("/var/lib/ambari-server/resources/stacks/NDP/2.1/services/SMARTSENSE/package/files/view/smartsense-ambari-view-1.4.0.0.60.jar"), "644", "{0}", False),
      (AmbariPath.get("/var/lib/ambari-server/resources/stacks/NDP/3.0/hooks/before-START/files/fast-hdfs-resource.jar"), "644", "{0}", False),
      # Also, /etc/ambari-server/conf/password.dat
      # is generated later at store_password_file
    ]
    self.NR_CHANGE_OWNERSHIP_LIST = [
